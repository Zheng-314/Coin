...
dic = {'01':0,'02':1,'03':2,'04':3, '06':4,'07':5,'09':6,'30':7,'35':8,'40':9,
      '45':10,'50':11,'53':12,'55':13,'58':14,'60':15,'61':16,
      '62':17,'62+':18,'63':19,'63+':20,'64':21,'64+':22,'65':23,'65+':24,'66':25}

category_counts = {category: 0 for category in l1}

def data_enhance(frow):
    save_file = "/media/data/workplace_wph/new/"
    path1, path2, class1 = frow.iloc[2], frow.iloc[3], frow.iloc[5]
    if class1 not in l3:
        return 
        
    label1 = dic[class1]
    # # 检查该类别的处理次数是否已达到300
    # if category_counts[class1] >= 300:
    #     return
    
    # # 增加该类别的处理计数
    # category_counts[class1] += 1
    ...
    # 正面数据集
    img1 = adjust_brightness(img1, random.uniform(0.7, 1.3))
    img1 = random_rotation(img1)
    gs1 = add_gaussian_noise(img1, sigma=50, prob = 0.030)
    sp1 = add_salt_pepper(img1, prob=0.015)
    
    f1 = path1.split('.')
    cv2.imwrite(save_file + f1[0] + "gs1." + f1[1], gs1)
    cv2.imwrite(save_file + f1[0] + "jy1." + f1[1], sp1)
    
    # 反面数据集
    img2 = adjust_brightness(img2, random.uniform(0.7, 1.3))
    img2 = random_rotation(img2)
    gs2 = add_gaussian_noise(img2, sigma=50, prob = 0.030)
    sp2 = add_salt_pepper(img2, prob=0.015)
    f2 = path2.split('.')
    cv2.imwrite(save_file + f2[0] + "gs1." + f2[1], gs2)
    cv2.imwrite(save_file + f2[0] + "jy1." + f2[1], sp2)

    
    with open("tdata.csv", "at") as fs:
        # 格式为 图片1、图片2、评级类别、类别标签
        fs.write(f"{path1},{path2},{class1},{label1}\n")
        fs.write(f"{f1[0] + 'gs1.' + f1[1]},{f2[0] + 'gs1.' + f2[1]},{class1},{label1}\n")
        fs.write(f"{f1[0] + 'jy1.' + f1[1]},{f2[0] + 'jy1.' + f2[1]},{class1},{label1}\n")
        fs.write(f"{f1[0] + 'gs1.' + f1[1]},{f2[0] + 'jy1.' + f2[1]},{class1},{label1}\n")
        fs.write(f"{f1[0] + 'gs1.' + f1[1]},{f2[0] + 'jy1.' + f2[1]},{class1},{label1}\n")
    ...
    f1 = path1.split('.')
    cv2.imwrite(save_file + f1[0] + "gs2." + f1[1], gs1)
    cv2.imwrite(save_file + f1[0] + "jy2." + f1[1], sp1)
    ...
    sp2 = add_salt_pepper(img2, prob=0.015)
    f2 = path2.split('.')
    cv2.imwrite(save_file + f2[0] + "gs2." + f2[1], gs2)
    cv2.imwrite(save_file + f2[0] + "jy2." + f2[1], sp2)

    
    with open("tdata.csv", "at") as fs:
        # 格式为 图片1、图片2、评级类别、类别标签
        fs.write(f"{path1},{path2},{class1},{label1}\n")
        fs.write(f"{f1[0] + 'gs2.' + f1[1]},{f2[0] + 'gs2.' + f2[1]},{class1},{label1}\n")
        fs.write(f"{f1[0] + 'jy2.' + f1[1]},{f2[0] + 'jy2.' + f2[1]},{class1},{label1}\n")
        fs.write(f"{f1[0] + 'gs2.' + f1[1]},{f2[0] + 'jy2.' + f2[1]},{class1},{label1}\n")
        fs.write(f"{f1[0] + 'gs2.' + f1[1]},{f2[0] + 'jy2.' + f2[1]},{class1},{label1}\n")
    # if class1 == 'VF':
...
if __name__ == "__main__":
    # data_enhance("1.jpg")
    df = pd.read_csv("coins.csv",sep = '\s+', engine='python')    
    print(df.head())
    # print(df.iloc[:, 0])
    # print(df.iloc[:, 1])
    # print(df.iloc[:, 3])
